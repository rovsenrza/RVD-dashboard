...
- src/features — dashboard, products, equipment (table or branch → department → machine → place tree), replacements (journal, ReplacementDialog), requests, compare (models side by side; manager+admin), reports (7 reports + /reports/print; manager+admin), notifications (page + prefs; the header bell reads it), admin (users, branches, settings, import, audit log; admin only), scan (camera/typed code → hose or machine), attachments (hose-card sheet, viewer, form picker + useUploads, history strip, paperclip count in tables), dev (UiPage — the /dev/ui catalogue, routed only when import.meta.env.DEV). Pages are composition only: PageHeader + QueryState + DataTable/components.
- src/app — layout/ (Layout, Sidebar rail, Header: branch switcher, ⌘K search, contact button, bell, user menu), session.tsx (SessionProvider/useSession mock until auth; demo role switch in the user menu, stored as rvd.role; branch-bound roles lock the branch), theme.ts (theme preference), router.tsx, providers.tsx.
  Routes (mock): GET /dashboard/summary, /products, /products/:id, /equipment, /replacements, /requests; POST /requests; admin (company-wide, never branch-scoped): /admin/users (GET/POST/PATCH, reset-password), /admin/branches, /admin/settings (warnPercent re-derives statuses), /admin/audit; POST /replacements (writes the old hose off, installs the new one in its place), GET /products/:id/replacements, GET /equipment/:id/replacements; GET /analytics/models; GET /reports/:id?branch&from&to; hose card: GET /products/:id/lifetime, /documentation, /comments (+POST, PATCH/DELETE /comments/:id); GET /notifications, POST /notifications/read, GET/PATCH /me/notification-prefs; files (Д25): GET /products/:id/attachments, POST /attachments (multipart; a draft without `productId`), GET /attachments/:id/file, DELETE /attachments/:id (hose files only); POST /requests and /replacements take `attachmentIds`.

## PATTERNS

- Two status axes (Д2): `ProductStatus` — health, the primary UI axis, derived only by `entities/product/rules.ts` (Д13; moves to packages/contracts/status.ts): start = installedAt, else shippedAt; «Внимание» = last `warnPercent` % of life or last `warnDays` before the planned date (`warnRule`, admin setting); the boundary day belongs to the later phase; statusOf/serviceDates/lifetimePhases/daysLeft serve mocks, BFF and UI alike; `ProductLifecycle` — the 1С workflow (Изготавливается → … → Списан), from ReleaseDocument history. Status text uses the -ink tier (AA). Notifications (Д19) come from a daily server job over the same phases and the company lead days; read state and per-user prefs live on the server.
...
- Empty values via EmptyValue; shaped skeletons; ErrorState with retry. Mocks read the demo role from `rvd.role` (the BFF: from the token).
...
- Every mutation writes an action-log line (who, when, object, fields before → after) as display strings; unchanged saves write nothing. Mocks record it in handlers; the BFF in middleware (Д23).
