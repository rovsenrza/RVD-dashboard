...
- src/features — dashboard (KpiGrid, ReplacementsChart, StatusDonut, UpcomingTable), products (ProductsPage, ProductPage, columns, ProductDetails), equipment, replacements (journal page, ReplacementDialog, ProductReplacements), requests, compare (ComparePage: machine models side by side, manager and admin), admin (AdminPage: users / branches / settings / import / audit-log tabs, admin role only; import/ holds pure row validation for users and installation facts), scan (ScanDialog + matchCode: camera or typed code → hose/machine by exact number), attachments (hose-card sheet, viewer, form picker + useUploads, history strip, paperclip count in tables), dev (UiPage — the /dev/ui catalogue, routed only when import.meta.env.DEV). Pages are composition only: PageHeader + QueryState + DataTable/components.
