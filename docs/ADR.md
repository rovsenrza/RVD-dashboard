...
- Status is computed from installedAt + serviceLifeDays (warn at last 20%), warranty separate; thresholds still to be agreed with customer (PLAN.md §5 q4). Text in a status colour always uses the -ink tier (AA).
- Domain model will become two-level (Рукав в сборе → Изделие) per docs/competitor-esm-analysis.md; add HoseAssembly + Product.assemblyId/esmNumber on Д2.
