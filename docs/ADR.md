...
- Two status axes (Д2): `ProductStatus` — health we compute from installedAt + serviceLifeDays (warn at the last warnPercent), the primary UI axis; `ProductLifecycle` — the 1С workflow (Изготавливается → … → Списан), from ReleaseDocument history. Status text uses the -ink tier (AA). Notifications (Д19) come from a daily server job over the same phases and the company lead days; read state and per-user prefs live on the server.
