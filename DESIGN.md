...
- **Notifications** (`/notifications`, every role, Д19): a list sheet beside a «Что присылать мне» sheet. The list has «Все · Непрочитанные» tabs and a rule select, then rows grouped under day headers («Сегодня», «Вчера», «22 сентября, вторник»; caption, uppercase). A row is the rule's 32px tile on its status `-soft` ground with the `-ink` icon (превышен срок: triangle, replace; плановая замена: calendar, warn; гарантия: shield, none; заявка: clipboard, neutral), the hose «EHS 48703 · HT08» (semibold while unread), the message, the time, and an amber dot while unread. Opening a row reads it and goes to the hose or the requests. The preferences are checkboxes per rule and channel, saved as they change; e-mail is disabled with the reason when the administrator turned it off; the lead days are the company's («Предупреждаем за 30, 14, 7 дней», linked to settings for the administrator). The header bell shows a red dot while anything is unread and opens a 24rem panel: «Уведомления · 3 новых» with «Прочитать все», six rows (unread first), and «Все уведомления».
- **Equipment** (`/equipment`, Д12): «Таблица · По подразделениям» switch in the header (URL `?view=tree`; its track takes `line/70` on the field so it reads as a control) beside «Экспорт»; one search feeds both views. The tree is one sheet per branch (branch title only when several are in scope): a department row (chevron, name, «3 машины · 48 РВД», its combined status bar right) opens by default to machine rows — chevron, garage number linking to the card, brand/model, type, hose count, nearest replacement, status bar — ordered attention first. A machine unfolds onto a `field` panel with its hoses by installation place: place name and count, then rows «EHS · catalogue number · status badge · N дн. до замены / просрочено» worst first, each opening the hose. The machine card shows the same place groups under «РВД по местам установки», and its description list adds заводской №, филиал, подразделение.
...
- **Header**: company › branch switcher (icon tile + two-line text + chevron), centred global search, amber «Связаться со специалистом», bell with red dot (unread notifications), avatar (amber, initials) + name + role + chevron menu; the menu holds «Тема» and, until real auth, «Роль · демо». For a branch-bound role (mechanic) the branch switcher becomes a static label with a lock and a tooltip instead of a menu.
