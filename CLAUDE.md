...
- Before commit run the whole CI gate, via the npm scripts and in this order: `npm run lint && npm run lint:arch && npm run format:check && npm run typecheck && npm test && npm run build`. Use the scripts, never an ad-hoc `tsc` — `npm run typecheck` is `tsc -b --noEmit` (build mode, project references) and catches errors a plain `tsc --noEmit -p .` misses. `format:check` covers the whole repo, not just `src`. When a change touches the UI, also run `npm run test:visual` (Playwright screenshots of the key screens in both themes, desktop and phone, against `e2e/__screenshots__`); if a diff is intended, review it and accept with `npm run test:visual:update`, committing the new baselines with the change. It is local only — baselines are macOS renders. Commit style: imperative subject, body explains why. Attribution line per the current system reminder.
